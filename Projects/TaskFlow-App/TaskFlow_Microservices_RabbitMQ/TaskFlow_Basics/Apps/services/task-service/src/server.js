...
let db, redisClient, rabbitChannel;
...
async function connectRabbitMQ() {
  try {
    if (!RABBITMQ_URL) {
      console.log('⚠️ RabbitMQ URL not provided, skipping');
      return;
    }
    const connection = await amqp.connect(RABBITMQ_URL);
    rabbitChannel = await connection.createChannel();
    await rabbitChannel.assertExchange('taskflow', 'topic', { durable: true });
...
async function publishEvent(eventType, data) {
  ...
  try {
    ...
    rabbitChannel.publish('taskflow', `task.${eventType}`, Buffer.from(message));
    console.log(`📤 Event published: task.${eventType}`);
  } catch (error) {
    console.error('Event publish error:', error.message);
  }
}
...
app.post('/api/tasks', async (req, res) => {
  ...
  try {
    ...
    await publishEvent('created', createdTask);
...
app.put('/api/tasks/:id', async (req, res) => {
  ...
  try {
    ...
    // Publish event to RabbitMQ
    await publishEvent('updated', task);
...
app.delete('/api/tasks/:id', async (req, res) => {
  ...
  try {
    ...
    // Publish event to RabbitMQ
    await publishEvent('deleted', { taskId: req.params.id });
